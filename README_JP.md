...
Registryは `display` 値を保持する。

```text
display = true
```

の場合、Ashi@はAshiatoを表示してよい。

```text
display = false
```

の場合、Ashi@はAshiatoを表示しない。

`display` の変更はAshi@上での表示だけに影響し、元のSNS投稿を削除・変更するものではない。
...
9. 非表示申請を自動化・大量送信されにくくする。
10. 一時的なセキュリティデータを短期間で失効させる。
