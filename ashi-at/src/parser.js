...
export function parseCandidate(c) {
  ...
  // FIX: the local context-id variable was previously named `c`, shadowing the
  // `c` function parameter (the full candidate string). `let c` re-declaring a
  // parameter of the same name is a SyntaxError in strict/module JS, so this
  // whole file failed to load. Renamed to `ctxId`.
  let i = 1,
    ctxId = null;
  if (t[i]?.startsWith("c:")) {
    ctxId = t[i].slice(2);
